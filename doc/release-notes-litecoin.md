Citadel Core version *0.15.1* is now available from:

  <https://download.citadel.org/citadel-0.15.1/>
...
A number of changes to the way Citadel Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 cities per kB (0.00001 CTD). This is to prepare for dropping the minimum transaction fee to 0.1 cities per kB (0.0001 CTD) in the next release.
